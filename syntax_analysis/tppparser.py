...
def p_cabecalho_error(p):
    ...
    print("Erro na definição do cabeçalho - Lista de parâmetros ou corpo.")

    print(f"Erro:p[0]:{p[0]}, p[1]:{p[1]}, p[2]:{p[2]}, p[3]:{p[3]}, p[3]:{p[4]}, p[3]:{p[5]}, p[3]:{p[6]}")
    error_line = p.lineno(2)
...
def p_parametro_error(p):
    ...
    """
    print("Erro na definição do parâmetro - Tipo ou parâmetro.")
...
def p_se_error(p):
    ...
    print("Erro na definição/condição 'SE' - Expressão ou corpo.")
...
def p_repita_error(p):
    ...
    print("Erro na definição/condição 'REPITA' - Expressão ou corpo.")

    print(f"Erro:p[0]:{p[0]}, p[1]:{p[1]}, p[2]:{p[2]}, p[3]:{p[3]}, p[4]:{p[4]}")
    error_line = p.lineno(2)
    father = MyNode(name=f'ERROR::{error_line}', type='ERROR')
    logging.error(f"Syntax error parsing index rule at line {error_line}")
    parser.errok()
...
def p_leia_error(p):
    ...
    print("Erro na definicao da expressão 'LEIA' - Variável.")
...
def p_fator_error(p):
    ...
    print("Erro na definião do fator - Expressão ou parâmetro.")

    if len(p) > 2:
        print(f"Erro:p[0]:{p[0]}, p[1]:{p[1]}, p[2]:{p[2]}, p[3]:{p[3]}")
    else:
        print(f"Erro:p[0]:{p[0]}, p[1]:{p[1]}")
    error_line = p.lineno(2)
    father = MyNode(name=f'ERROR::{error_line}', type='ERROR')
    logging.error(f"Syntax error parsing index rule at line {error_line}")
    parser.errok()
